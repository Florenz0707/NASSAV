...
- **关系映射推荐器**：新增基于多重映射探索的推荐器，建立起 `genre` <-> `source_category` 以及 `actor_name` <-> `source_actor_name` 等持久化映射表，实现强大的探索推荐发现闭环。
- **推荐反馈式学习机制**：为推荐链路引入了反馈抑制驱动池，并由此增强了对由于同质化推荐导致的不良返回的抑制，全面强化了探索与内容召回效率。
- **推荐个性化调优控制**：前端针对推荐模块新增并适配了独立完整的“推荐策略与个性化筛选弹窗”（Personalization Window）面板。

#### 🎨 优化 & 重构

- **前端全局页面设计规范落实**：从零梳理全站色彩变量并切换为完整的 TailwindCSS 调色板映射（如 `var(--accent-primary)`, `var(--bg-secondary)`），全盘移除散落在 CSS 或内联样式内的 Hex 颜色硬接写，同时弃用不可靠的 Unicode 与 Emoji，统一切换至标准矢量 SVG 图标方案。
- **重构推荐相关数据表结构**：对近期开发中因迭代过快导致的凌乱数据结构进行了合并和删改调整，并在爬取抓取、资源追加等生命周期中挂载双向映射回填同步逻辑。
- **组件标准化下沉**：深度调整组件化粒度并提取了可复用的视觉实体模块（涉及 `RecommendationCard` 等各种子卡片及悬浮框组）。

#### 🐛 修复

- 修复了因为刷新解析抓取 M3U8 播放链接，可能导致原有手工编撰的本地非源字段被意外全量置空的 Bug。
- 修复了正在下载任务可能偶发在下载界面显示实时速度异常呈现为 `N/A` 的面板问题。
- 修复了后端无法正常爬取翻读 Jable 的下一分页记录，以及关联引发的数据库迁移映射数据空洞问题。
