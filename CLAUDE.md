# 项目开发规范（FOR AGENT）

## 普遍规范

- 沟通的时候使用中文，尽量减少表情类emoji的使用
- 除非用户有明确要求，否则不应该生成新文档
- 完成一次编写代码后，使用`pre-commit run -a`来修复可能存在的问题
- Commit Message格式：[Summary] Brief English text to summarize what has been done, no code quotation or emoji.

## 后端开发

### 编写代码前

- 使用`django_backend`作为后端开发根目录
- 阅读`django_backend`下的项目代码，理解项目结构和功能
- 本项目使用`uv`管理虚拟环境，若需要运行python脚本，请运行`uv run script.py`
- `config/`是项目配置目录目录，若需要新增/删除配置，请同步修改`template-config.yaml`
- `doc`是项目文档目录，其中包含[接口说明](doc/interface.md)，[数据库说明](doc/database.md)和[测试端点说明](doc/debug.md)。若项目有改动，请同步更新它们
- `scripts/`是项目脚本目录，用于批量修改数据或其他用途。脚本应该把支持的参数写在文件开头，并将用法更新到[文档](scripts/README.md)
- `tests/`是项目测试目录，用于测试项目的可用性和正确性。脚本应该把支持的参数写在文件开头，并将用法更新到[文档](scripts/README.md)。使用`uv run pytest tests/ -tb=short | tail -10`进行集成测试

### 编写代码时

- 模块业务的责任应保持最小且互不耦合
- 模块业务必须具有一个抽象基类，负责提供各种模板方法让子类实现
- 子类不应被外部调用基类未声明的方法
- 注意保持代码的**高可用**、**可扩展**、**易阅读**性

### 编写代码后

- 运行单元测试和集成测试
- 更新上述所说文档

## 前端开发

### 编写代码前

- 使用`vue_frontend`作为前端开发根目录
- 阅读`vue_frontend`下的项目代码，理解项目结构和功能
- 本项目使用`pnpm`管理

### 编写代码时

- 遵守**合理封装**，**适当复用**、**开放修改**的代码设计原则
- 沿用目前的UI风格，包括组件格式、主题色和字体等

### 编写代码后

- 使用`pnpm run build | tail -10`测试代码的正确性
- 使用`pnpm run lint:fix`测试代码的风格规范
