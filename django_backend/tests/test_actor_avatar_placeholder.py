...
def test_actor_with_placeholder_avatar_returns_none(
    client, actor_factory, resource_factory
):
    """测试有占位图的演员返回 None（使用 fixtures）"""
    resource = resource_factory(avid="TEST-001", original_title="Test")
    actor = actor_factory(
        name="测试演员",
        avatar_url=ACTOR_AVATAR_PLACEHOLDER_URLS[0],
...
@pytest.mark.django_db
def test_actor_with_valid_avatar_returns_url(client, actor_factory, resource_factory):
    """测试有真实头像的演员正常返回（使用 fixtures）"""
    resource = resource_factory(avid="TEST-002", original_title="Test")
    actor = actor_factory(
        name="真实演员",
...
@pytest.mark.django_db
def test_actor_without_avatar_returns_none(client, actor_factory, resource_factory):
    """测试没有头像的演员返回 None（使用 fixtures）"""
    resource = resource_factory(avid="TEST-003", original_title="Test")
    actor = actor_factory(name="无头像演员", avatar_url=None, avatar_filename=None)
    resource.actors.add(actor)
...
@pytest.mark.django_db
def test_mixed_actors_filters_correctly(client, actor_factory, resource_factory):
    """测试混合场景：占位图、真实头像、无头像（使用 fixtures）"""
    resource = resource_factory(avid="TEST-004", original_title="Test")

    actor1 = actor_factory(
        name="占位图演员",
        avatar_url="https://pics.dmm.co.jp/mono/actjpgs/nowprinting.gif",
        avatar_filename="nowprinting.gif",
    )
    actor2 = actor_factory(
        name="真实头像演员",
        avatar_url="https://www.javbus.com/pics/actress/456_a.jpg",
        avatar_filename="456_a.jpg",
    )
    actor3 = actor_factory(name="无头像演员", avatar_url=None, avatar_filename=None)
