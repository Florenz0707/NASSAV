<template>
    <div class="actors-page">
        <div class="mb-8">
            <h1 class="text-[2rem] font-bold text-[#f4f4f5] mb-2">演员库</h1>
            <p class="text-[#71717a] text-base">查看所有演员信息</p>
        </div>
            ...
            <div class="flex items-center justify-center gap-4">
                <div class="flex items-center gap-2">
                    <button
                        ...
                        @click="loadPage(page)">跳转至第</button>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input v-model.number="pageSize" @change="onPageSizeChange" type="number" min="1"
                        class="w-20 px-3 py-1 rounded-md bg-[#1b1b26] text-[#f4f4f5] border border-white/[0.06] focus:outline-none text-center" />
                    <label class="text-sm text-[#bcbcbc]">个演员卡</label>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    setup() {
        ...
        return { store, openActor, prev, next, page, pageSize, pageSizeOptions, onPageSizeChange, loadPage, searchQuery, sortBy, sortOrder, onSortChange }
    }
}
</script>
