        ...
        <label class="inline-flex items-center gap-3 text-sm text-[#bcbcbc]">
          <label class="inline-flex items-center cursor-pointer">
            <input type="checkbox" class="sr-only"
              :checked="selectedCount === displayedResources.length && displayedResources.length > 0"
              @change="toggleSelectAll($event.target.checked)" />
            <span
              :class="['w-5 h-5 flex items-center justify-center rounded-md transition border-2', (selectedCount === displayedResources.length && displayedResources.length > 0) ? 'bg-gradient-to-br from-[#ff6b6b] to-[#ff5252] border-white shadow' : 'bg-[rgba(128,128,128,0.6)] border-white text-white']">
              <svg v-if="selectedCount === displayedResources.length && displayedResources.length > 0"
