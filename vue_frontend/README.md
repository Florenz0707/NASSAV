...
一个用于管理视频资源的前端应用，包含资源检索、信息刮削、下载管理等功能。基于 Vue 3、Vite、Pinia 与 Vue Router 构建，默认通过 `/nassav` 代理与后端交互。
...
- 开发代理：参见 [vite.config.js](vite.config.js)，`/nassav` 将被代理到 `http://localhost:8000`。请确保后端在本地 8000 端口可用，或按需修改配置。
